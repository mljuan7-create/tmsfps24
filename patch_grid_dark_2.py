...
export const SalasGrid: React.FC<SalasGridProps> = ({
  ...
  return (
    <section className="flex flex-col h-full">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-xl bg-zinc-800/80 text-zinc-300 hover:bg-zinc-700 hover:text-white border border-zinc-700/50 transition shadow-sm"
          >
            {todasSeleccionadas ? (
      ...
      {/* Grid de Salas - Estilo Más Cuadrado y Redondeado */}
      <div className="grid grid-cols-1 lg:grid-cols-2 2xl:grid-cols-3 gap-5">
        {salas.map((sala) => {
          ...
          const barColor = isPlaying 
            ? (lucesEncendidas ? 'bg-amber-400 shadow-[0_0_12px_rgba(251,191,36,0.4)]' : 'bg-[#12b886] shadow-[0_0_12px_rgba(18,184,134,0.3)]')
            : isPaused ? 'bg-amber-600' : 'bg-zinc-600';

          return (
            <div 
              key={sala.id}
              className={`relative flex bg-zinc-900/80 rounded-2xl transition-all duration-300 border overflow-hidden group hover:border-zinc-700 cursor-pointer ${
                isSelected ? 'border-sky-500 shadow-[0_0_20px_rgba(14,165,233,0.15)] bg-sky-950/10' : 'border-zinc-800/80 shadow-md'
              }`}
            ...
            >
              ...
              <button 
                ...
                className="absolute top-4 right-4 p-2 rounded-xl bg-zinc-800 text-zinc-400 opacity-0 group-hover:opacity-100 transition-all hover:bg-sky-500 hover:text-white z-20 shadow-lg scale-90 group-hover:scale-100"
              ...
              <div className="w-[120px] bg-black/40 flex-shrink-0 flex flex-col items-center justify-center border-r border-zinc-800/80 relative p-4 gap-3">
                 <span className={`text-6xl font-black tracking-tighter ${sala.cpl_actual ? 'text-zinc-700/80' : 'text-zinc-800/50'}`}>
                    {sala.id}
                 </span>
                 {sala.cpl_actual ? (
                    <Film className={`w-7 h-7 ${isPlaying ? (lucesEncendidas ? 'text-amber-400' : 'text-[#12b886]') : isPaused ? 'text-amber-600' : 'text-zinc-500'}`} />
                 ) : (
                    <Film className="w-7 h-7 text-zinc-800" />
                 )}
              </div>
              ...
              <div className="flex-1 p-5 flex flex-col justify-between">
                ...
                <div className="pr-10">
                  <h3 className={`text-[16px] font-semibold tracking-wide truncate ${sala.cpl_actual ? 'text-zinc-100' : 'text-zinc-600'}`}>
                    {sala.cpl_actual ? sala.cpl_actual.replace(/_/g, ' ') : 'SALA LIBRE'}
                  </h3>
                  {sala.cpl_actual && (
                    <div className="flex items-center gap-2 mt-1.5 text-xs text-zinc-500 font-mono font-medium">
                      <span className={`w-2 h-2 rounded-full ${isPlaying ? (lucesEncendidas ? 'bg-amber-400 animate-pulse' : 'bg-[#12b886] animate-pulse') : isPaused ? 'bg-amber-600' : 'bg-zinc-500'}`}></span>
                      {lucesEncendidas ? 'DESALOJO (LUCES ON)' : sala.estado_reproduccion}
                    </div>
                  )}
                </div>
                ...
                {sala.cpl_actual ? (
                  <div className="mt-4 mb-1">
                    <div className="flex items-center justify-between text-xs font-mono mb-2 text-zinc-400 font-medium">
                    ...
                    <div className="relative w-full h-2.5 bg-zinc-800/80 rounded-full overflow-visible">
                      ...
                      {duracion > 5 && (
                        <div 
                          className={`absolute top-1/2 -translate-y-1/2 w-1.5 h-4 rounded-full z-10 transition-colors ${lucesEncendidas ? 'bg-amber-200' : 'bg-zinc-400'}`}
                    ...
                    <div className="flex items-center justify-between mt-4">
                      <div className="flex items-center gap-1.5 text-[11px] uppercase font-bold tracking-wider">
                         <Sun className={`w-4 h-4 ${lucesEncendidas ? 'text-amber-400 fill-amber-400 animate-pulse' : 'text-zinc-500'}`} />
                         {lucesEncendidas ? (
                            <span className="text-amber-400">Gente Saliendo</span>
                         ) : isPlaying ? (
                      ...
                      <div className="flex items-center gap-1.5" onClick={(e) => e.stopPropagation()}>
                        <button onClick={() => onEnviarComandoSala?.(sala.id, 'play')} className={`p-2 rounded-lg transition ${isPlaying ? 'bg-zinc-800 text-[#12b886]' : 'bg-zinc-800/50 hover:bg-zinc-700 text-zinc-500'}`}>
                          <Play className="w-3.5 h-3.5 fill-current" />
                        </button>
                        <button onClick={() => onEnviarComandoSala?.(sala.id, 'pause')} className={`p-2 rounded-lg transition ${isPaused ? 'bg-zinc-800 text-amber-500' : 'bg-zinc-800/50 hover:bg-zinc-700 text-zinc-500'}`}>
                          <Pause className="w-3.5 h-3.5 fill-current" />
                        </button>
                        <button onClick={() => onEnviarComandoSala?.(sala.id, 'stop')} className={`p-2 rounded-lg transition bg-zinc-800/50 hover:bg-red-500/20 hover:text-red-400 text-zinc-500`}>
                ...
                {/* Próxima Sesión */}
                <div className="mt-3 pt-3 border-t border-zinc-800/50 flex items-center justify-between text-[11.5px] text-zinc-500 font-medium">
                  <span className="flex items-center gap-1.5">
                    <Clock className="w-4 h-4 opacity-70" />
                    Siguiente: <span className="text-zinc-400">{sala.cpl_actual ? (sala.id % 2 === 0 ? 'INSIDIOUS' : 'TADEO JONES') : 'Pendiente'}</span>
                  </span>
                  <span className="font-mono bg-zinc-800/50 px-2 py-0.5 rounded text-zinc-400">{getNextSessionTime(sala.id)}</span>
                </div>
              </div>
            </div>
          );
